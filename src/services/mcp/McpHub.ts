...
export class McpHub {
	...
	private async watchProjectMcpFile(): Promise<void> {
		...
		const projectMcpPattern = new vscode.RelativePattern(workspaceFolder, ".testagent/mcp.json") // test-agent_change (was .kilocode)
  ...
  "mcpServers": {
	...
	private async getProjectMcpPath(): Promise<string | null> {
		...
		const projectMcpDir = path.join(workspacePath, ".testagent") // test-agent_change (was .kilocode)
		...
		try {
			...
			// If not found in .testagent/, fall back to .mcp.json in root directory
