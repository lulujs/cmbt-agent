	...
	*/
	...
	async ensureMcpServersDirectoryExists(): Promise<string> {
		...
		try {
			...
			return path.join(os.homedir(), ".testagent", "mcp") // test-agent_change (was .kilocode)
		}
	...
	async handleKiloCodeCallback(token: string) {
		...
		vscode.window.showInformationMessage("TEST Agent successfully configured!")
	...
	> {
		...
		return {
			...
			language: stateValues.language ?? "zh-CN", // kilocode_change: default to Chinese
			mcpEnabled: true, // kilocode_change: always true
	...
	async resetState() {
		...
		// Logout from Test Agent provider before resetting (same approach as ProfileView logout)
		const { apiConfiguration, currentApiConfigName = "default" } = await this.getState()
