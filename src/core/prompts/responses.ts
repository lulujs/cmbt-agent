	...
	rooIgnoreError: (path: string, protocol?: ToolProtocol) => {
		if (isNativeProtocol(protocol ?? TOOL_PROTOCOL.XML)) {
			return JSON.stringify({
				status: "error",
				type: "access_denied",
				message: "Access blocked by .testagentignore",
				path: path,
				suggestion: "Try to continue without this file, or ask the user to update the .testagentignore file",
			})
		}
		return `Access to ${path} is blocked by the .testagentignore file settings. You must try to continue in the task without using this file, or ask the user to update the .testagentignore file.`
	},
