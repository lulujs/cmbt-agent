...
description: "Integrate Test Agent with your shell environment"
---

# Terminal Shell Integration

Terminal Shell Integration is a key feature that enables Test Agent to execute commands in your terminal and intelligently process their output. This bidirectional communication between the AI and your development environment unlocks powerful automation capabilities.

## What is Shell Integration?

Shell integration is automatically enabled in Test Agent and connects directly to your terminal's command execution lifecycle without requiring any setup from you. This built-in feature allows Test Agent to:
...
When Test Agent needs to perform tasks like installing dependencies, starting a development server, or analyzing build errors, shell integration works behind the scenes to make these interactions smooth and effective.

## Getting Started with Shell Integration

Shell integration is built into Test Agent and works automatically in most cases. If you see "Shell Integration Unavailable" messages or experience issues with command execution, try these solutions:
...
Test Agent provides several settings to fine-tune shell integration. Access these in the Test Agent panel under Settings → Terminal.
...
Adds a small pause after running commands to help Test Agent capture all output correctly. This setting can significantly impact shell integration reliability due to VSCode's implementation of terminal integration across different operating systems and shell configurations:
...
Helps PowerShell run the same command multiple times in a row. Enable this if you notice Test Agent can't run identical commands consecutively in PowerShell.
...
Prevents ZSH from adding special characters at the end of output lines that can confuse Test Agent when reading terminal results.
...
Makes Test Agent work better with the popular [Oh My Zsh](https://ohmyz.sh/) shell customization framework. Turn this on if you use Oh My Zsh and experience terminal issues.
...
Improves compatibility if you use the Powerlevel10k theme for ZSH. Turn this on if your fancy terminal prompt causes issues with Test Agent.
...
Helps Test Agent work with custom ZSH configurations without interfering with your personal shell settings and customizations.
...
**Alternative**: Enable the Powerlevel10k Integration setting in Test Agent.
...
1. **Set Terminal Command Delay**:
    ...
    - Set the Terminal Command Delay to 50ms in Test Agent settings
...
4. **ZSH Users**:
    - Try enabling some or all ZSH-related workarounds in Test Agent settings
...
**Issue**: If text is already typed in the terminal when Test Agent tries to run a command, Test Agent will press Ctrl+C first to clear the line, which can interrupt running processes.

**Workaround**: Make sure your terminal prompt is empty (no partial commands typed) before asking Test Agent to execute terminal commands.

### Multi-line Command Issues

**Issue**: Commands that span multiple lines can confuse Test Agent and may show output from previous commands mixed in with current output.
...
1. **Premature Completion**: PowerShell sometimes tells Test Agent a command is finished before all the output has been shown.
