...
Auto-Launch Configuration allows you to automatically start a Test Agent task when opening a workspace, with support for specific profiles and modes. This was originally developed as an internal test feature, but we decided to expose it to users in case anyone finds it useful!
...
When you open a workspace in VS Code, Test Agent automatically checks for a launch configuration JSON file. If found, it:
...
- **`mode`** (string): The Test Agent mode to use for this task. Available modes:
...
- Auto-launch triggers approximately 500ms after Test Agent extension activation
...
1. Verify file location: `.testagent/launchConfig.json` in workspace root
