...
Model Context Protocol (MCP) supports two primary transport mechanisms for communication between Test Agent and MCP servers: Standard Input/Output (STDIO) and Server-Sent Events (SSE). Each has distinct characteristics, advantages, and use cases.
...
1. The client (Test Agent) spawns an MCP server as a child process
...
- **Locality**: Runs on the same machine as Test Agent
...
- Single-client scenarios (one Test Agent instance per server)
...
1. The client (Test Agent) connects to the server's SSE endpoint via HTTP GET request
...
- **Remote Access**: Can be hosted on a different machine from Test Agent
...
STDIO servers run locally on the same machine as Test Agent, which has several important implications:
...
- **Execution**: Starts and stops with Test Agent (child process lifecycle)
...
- Start when needed by Test Agent
- Not require network configuration
- Need to be installed alongside Test Agent or via a package manager
...
## Configuring Transports in Test Agent

For detailed information on configuring STDIO and SSE transports in Test Agent, including example configurations, see the [Understanding Transport Types](using-in-kilo-code#understanding-transport-types) section in the Using MCP in Test Agent guide.
