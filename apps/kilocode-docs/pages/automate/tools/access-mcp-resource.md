...
The `access_mcp_resource` tool retrieves data from resources exposed by connected Model Context Protocol (MCP) servers. It allows Test Agent to access files, API responses, documentation, or system information that provides additional context for tasks.
...
- When Test Agent needs additional context from external systems
- When Test Agent needs to access domain-specific data from specialized MCP servers
- When Test Agent needs to retrieve reference documentation hosted by MCP servers
- When Test Agent needs to integrate real-time data from external APIs via MCP
...
4. **Response Processing**:
    ...
    - Returns the processed resource data to Test Agent for use in the current task
...
- When helping with API development, Test Agent retrieves endpoint specifications from MCP resources to ensure correct implementation.
- When assisting with data visualization, Test Agent accesses current data samples from connected MCP servers.
- When working in specialized domains, Test Agent retrieves technical documentation to provide accurate guidance.
- When generating industry-specific code, Test Agent references compliance requirements from documentation resources.
