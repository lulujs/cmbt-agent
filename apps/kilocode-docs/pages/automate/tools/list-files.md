...
The `list_files` tool displays the files and directories within a specified location. It helps Test Agent understand your project structure and navigate your codebase effectively.
...
- When Test Agent needs to understand your project structure
- When Test Agent explores what files are available before reading specific ones
- When Test Agent maps a codebase to better understand its organization
- Before using more targeted tools like `read_file` or `search_files`
- When Test Agent needs to check for specific file types (like configuration files) across a project
...
- When starting a new task, Test Agent may list the project files to understand its structure before diving into specific code.
- When asked to find specific types of files (like all JavaScript files), Test Agent first lists directories to know where to look.
- When providing recommendations for code organization, Test Agent examines the current project structure first.
- When setting up a new feature, Test Agent lists related directories to understand the project conventions.
