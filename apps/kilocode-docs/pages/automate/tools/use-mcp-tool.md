...
The `use_mcp_tool` tool enables interaction with external tools provided by connected Model Context Protocol (MCP) servers. It extends Test Agent's capabilities with domain-specific functionality through a standardized protocol.
...
This tool allows Test Agent to access specialized functionality provided by external MCP servers. Each MCP server can offer multiple tools with unique capabilities, extending Test Agent beyond its built-in functionality. The system validates arguments against schemas, manages server connections, and processes responses of various content types (text, image, resource).
...
3. **Response Processing**:
    ...
    - Results are formatted for display in the Test Agent interface
