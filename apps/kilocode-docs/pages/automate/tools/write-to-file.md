...
- When Test Agent needs to create a new file from scratch
- When Test Agent needs to completely rewrite an existing file
...
- Access Control: Validates against `.testagentignore` restrictions before making changes
...
1. **Parameter Validation**: Validates the required parameters and permissions
    ...
    - Validates the file is allowed (not restricted by `.testagentignore`)
...
- When creating a new project, Test Agent generates multiple files but lets you review each before committing changes.
- When setting up configuration files, Test Agent shows the proposed configuration in a diff view for approval.
- When generating documentation, Test Agent creates markdown files but lets you make final adjustments in the diff view.
- When developing a prototype, Test Agent shows complete source files in a diff view where you can fine-tune before saving.
