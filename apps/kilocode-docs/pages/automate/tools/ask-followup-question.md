...
This tool creates a conversational interface between Test Agent and the user, allowing for gathering clarification, additional details, or user preferences when facing ambiguities or decision points. Each question can include suggested responses to streamline the interaction.

## When is it used?

- When critical information is missing from the original request
- When Test Agent needs to choose between multiple valid implementation approaches
- When technical details or preferences are required to proceed
- When Test Agent encounters ambiguities that need resolution
...
1. **Information Gap Recognition**: Test Agent identifies missing information needed to proceed
2. **Specific Question Creation**: Test Agent formulates a clear, targeted question
3. **Suggestion Development**: Test Agent creates relevant suggested answers (optional but recommended)
...
10. **Task Continuation**: Test Agent proceeds with the task using the new information

## Examples When Used

- When developing a web application, Test Agent might ask about preferred styling frameworks (Bootstrap, Tailwind, custom CSS)
- When creating an API, Test Agent might ask about authentication methods (JWT, OAuth, API keys)
- When refactoring code, Test Agent might ask about prioritizing performance vs. readability
- When setting up a database, Test Agent might ask about specific schema design preferences
- When creating a custom feature, Test Agent might ask about specific behavior expectations
- When troubleshooting errors, Test Agent might ask about specific environment details
