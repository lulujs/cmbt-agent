...
The `browser_action` tool enables web automation and interaction via a Puppeteer-controlled browser. It allows Test Agent to launch browsers, navigate to websites, click elements, type text, and scroll pages with visual feedback through screenshots.
...
This tool creates an automated browser session that Test Agent can control to navigate websites, interact with elements, and perform tasks that require browser automation. Each action provides a screenshot of the current state, enabling visual verification of the process.

## When is it used?

- When Test Agent needs to interact with web applications or websites
...
- Browser sessions are temporary and not persistent across Test Agent restarts
...
- When creating a web form submission process, Test Agent launches a browser, navigates to the form, fills out fields with the `type` action, and clicks submit.
- When testing a responsive website, Test Agent navigates to the site and uses scroll actions to examine different sections.
- When capturing screenshots of a web application, Test Agent navigates through different pages and takes screenshots at each step.
- When demonstrating an e-commerce checkout flow, Test Agent simulates the entire process from product selection to payment confirmation.
