...
- When Test Agent needs to make precise changes to existing code without rewriting entire files.
...
- Validates file access against `.testagentignore` rules.
...
2.  **KiloCodeIgnore Check**: Validates if the target file path is allowed by `.testagentignore` rules.
...
Test Agent uses this strategy for applying diffs:
