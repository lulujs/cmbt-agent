---
title: "Custom Rules"
description: "Define custom rules for Test Agent behavior"
---

# Custom Rules

Custom rules provide a powerful way to define project-specific and global behaviors and constraints for the Test Agent AI agent. With custom rules, you can ensure consistent formatting, restrict access to sensitive files, enforce coding standards, and customize the AI's behavior for your specific project needs or across all projects.
...
Test Agent supports two types of custom rules:
...
Test Agent provides a built-in interface for managing your custom rules without manually editing files in the `.kilocode/rules/` directories. To access the UI, click on the <Codicon name="law" /> icon in the **bottom right corner** of the Test Agent window.
...
{% image src="/docs/img/custom-rules/rules-ui.png" alt="Rules tab in Test Agent" width="400" /%}

The easiest way to create and manage rules is through the built-in UI:

1. Access the rules management interface from the Test Agent panel
...
Rules will be automatically applied to all future Test Agent interactions. Any new changes will be applied immediately.
...
{% image src="/docs/img/custom-rules/custom-rules.png" alt="Test Agent ignores request to read sensitive file" width="600" /%}
