...
# Using ChatGPT Subscriptions With Test Agent
...
5. Back in Test Agent settings, pick a model from the dropdown.
6. Save.
...
- **You can't use arbitrary OpenAI API models.** This provider only exposes the models listed in Test Agent's Codex model catalog.
- **You can't export/migrate your sign-in state with settings export.** OAuth tokens are stored in VS Code SecretStorage, which isn't included in Test Agent's settings export.
