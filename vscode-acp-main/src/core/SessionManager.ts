...
export class SessionManager extends EventEmitter {
	...
	async connectToAgent(agentName: string): Promise<SessionInfo> {
		...
		try {
			...
			sendError(
				"agent/connect.end",
				{ agentName, result: "error", errorMessage: e.message || String(e) },
				{ duration: Date.now() - connectStartTime },
			)
			throw e
		}
	 ...
	 */
	private async createAcpSession(agentName: string, agentId: string, connInfo: ConnectionInfo): Promise<SessionInfo> {
		...
		try {
			...
			const isAuthRequired =
				(e instanceof RequestError && e.code === -32000) ||
				e?.code === -32000 ||
		...
		return {
			...
			agentName,
			agentDisplayName:
				connInfo.initResponse.agentInfo?.title || connInfo.initResponse.agentInfo?.name || agentName,
			cwd,
